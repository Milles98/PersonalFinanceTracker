...
public class MainViewModel : INotifyPropertyChanged
{
    ...
    public MainViewModel(IUserService userService, ITransactionService transactionService)
    {
    ...
    {
        IsLoggedIn = false;
        CurrentView = new LoginView { DataContext = new LoginViewModel(_userService, ShowWelcomeView) };
    }
    ...
    private void ShowTransactionEntryView()
    {
        if (IsLoggedIn)
        {
            CurrentView = new TransactionEntryView { DataContext = new TransactionEntryViewModel(_transactionService, _currentUserId) };
        }
    }
    ...
    private void ShowTransactionHistoryView()
    {
        if (IsLoggedIn)
        {
            CurrentView = new TransactionHistoryView { DataContext = new TransactionHistoryViewModel(_transactionService, _currentUserId, ShowTransactionUpdateView) };
        }
    }
